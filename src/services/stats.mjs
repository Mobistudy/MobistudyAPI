...
/**
 * Combines two means given their counts
 * @param {number} mean1
 * @param {number} n1
 * @param {number} mean2
 * @param {number} n2
 ...
 */
export function combineMeans (mean1, n1, mean2, n2) {
  return (mean1 * n1 + mean2 * n2) / (n1 + n2)
}

/** combines two variances given their means and counts
 * @param {number} mean1
 * @param {number} var1
 * @param {number} n1
 * @param {number} mean2
 * @param {number} var2
 * @param {number} n2
 * @returns the combined variance
 */
export function combineVariances (mean1, var1, n1, mean2, var2, n2) {
  let combinedMean = combineMeans(mean1, n1, mean2, n2)
  let combinedVar = ((n1 - 1) * var1) + ((n2 - 1) * var2) + (n1 * (mean1 - combinedMean) ** 2) + (n2 * (mean2 - combinedMean) ** 2)
  combinedVar /= (n1 + n2 - 1)
  return combinedVar
}
